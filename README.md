...
In the inference script **inference.py**, you can manually set the image path of the reference image, as well as the target text prompt to describe the scene content of the generated illusion image. The parameters "direct_transfer_steps", "decayed_transfer_steps", "async_ahead_steps", "exponent" in the <strong>sample_illusion_image</strong> function, as well as the parameters "contrast", "add_noise", "noise_value" in the <strong>load_ref_img</strong> are all tunable
